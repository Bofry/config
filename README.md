...
| type                  | struct tag | tag flags  | ConfigurationService method    | example |
|:----------------------|:-----------|:-----------|:-------------------------------|:--------|
...
| command arguments     | `arg`      | *required* | LoadCommandArguments()         | `arg:"SERVER_NAME"` -or- `arg:"SERVER_NAME;specify server name"`


$~$
### Environment Variables
⠿ The following struct will import environment variables `CACHE_HOST`, `CACHE_PASSWORD`, and `CACHE_DB`. The tag text `env:"CACHE_HOST,required"` use the flag *required* indicates the environment variable `CACHE_HOST` is required. It will get exception if the variable doesn't be assgined.
```go
type Config struct {
...
```
The text `env:"CACHE_HOST,required"` can be switch as `env:"*CACHE_HOST"` as well. Put the symbol "`*`" in front of the name is equivalent to appending `required` to tag flag part. 
```go
type Config struct {
...
$~$
### Resource Files
⠿ The following struct will import content from file `VERSION`. The tag text `resource:"VERSION,required"` use the flag *required* indicates the file `VERSION` is required. It will get exception if the file doesn't exist.
```go
type Config struct {
  AppVersion string `resource:"VERSION,required"`
}
```
The text `resource:"VERSION,required"` can be switch as `resource:"*VERSION"` as well. Put the symbol "`*`" in front of the name is equivalent to appending `required` to tag flag part. 
```go
type Config struct {
...
$~$
### Command Arguments
⠿ The following struct will import command arguments `cache-host`, `cache-passowrd`, and `cache-db`. The tag text `arg:"cache-host;the cache server address and port"` separated by symbol "`;`" to two parts. The name part and the usage text part for help.
```go
type Config struct {
